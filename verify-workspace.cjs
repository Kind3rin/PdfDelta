...
async function main() {
  ...
  try {
    ...
    await evaluate(`(async () => {
      ...
      if (!$('editorUndoInsertion').disabled || !$('editorRedoInsertion').disabled || $('editorShowOriginal').getAttribute('aria-pressed') !== 'false') throw new Error('New document retained editor history');
      const ink = () => { const canvas = $('editorInkCanvas'); return canvas.getContext('2d').getImageData(0,0,canvas.width,canvas.height).data.reduce((sum,value,i) => sum + (i % 4 === 3 ? value : 0), 0); };
      const ready = async page => { const deadline = Date.now()+10000; while ($('editorPageInfo').textContent !== page+' / 2') { if(Date.now()>deadline) throw new Error('Editor history page restore timeout'); await new Promise(r=>setTimeout(r,20)); } };
      ...
      const bothSignatures = ink();
      if ($('editorUndoInsertion').disabled) throw new Error('Undo insertion unavailable');
      await $('editorUndoInsertion').onclick();
      if ($('editorRedoInsertion').disabled || ink() >= bothSignatures) throw new Error('Editor undo failed');
      await $('editorRedoInsertion').onclick();
      if (ink() !== bothSignatures) throw new Error('Editor redo did not restore drawn signature');
      await $('editorUndoInsertion').onclick();
      $('signatureSize').value = '240'; place(.99,.99);
      if (!$('editorRedoInsertion').disabled) throw new Error('New annotation retained abandoned redo branch');
      ...
      const pageTwoInk = ink();
      $('editorShowOriginal').click();
      if (ink() !== 0 || $('editorShowOriginal').getAttribute('aria-pressed') !== 'true' || !$('editorUndoInsertion').disabled || !$('editorClear').disabled) throw new Error('Original comparison did not hide annotations');
      place(.6,.7);
      $('editorShowOriginal').click();
      if (ink() !== pageTwoInk) throw new Error('Original comparison changed annotations');
      $('editorClear').click();
      if (ink() !== 0) throw new Error('Clear current page left annotations');
      $('editorPrev').click(); await ready(1);
      await $('editorUndoInsertion').onclick();
      if ($('editorPageInfo').textContent !== '2 / 2' || ink() !== pageTwoInk) throw new Error('Clear undo failed to restore the correct page');
      await $('editorRedoInsertion').onclick();
      if (ink() !== 0) throw new Error('Clear redo failed');
      await $('editorUndoInsertion').onclick();
      document.querySelector('[data-editor-mode="draw"]').click();
      const pointer = (type,x,y) => { const r=$('editorInkCanvas').getBoundingClientRect(); $('editorInkCanvas').dispatchEvent(new PointerEvent(type,{pointerId:71,clientX:r.x+r.width*x,clientY:r.y+r.height*y,bubbles:true})); };
      pointer('pointerdown',.6,.7); pointer('pointerup',.6,.7);
      if ($('editorRedoInsertion').disabled || ink() !== pageTwoInk) throw new Error('A tap without a stroke changed history');
      pointer('pointerdown',.6,.7); pointer('pointermove',.7,.75);
      window.dispatchEvent(new Event('resize')); await ready(2);
      if (!$('editorRedoInsertion').disabled || ink() <= pageTwoInk) throw new Error('Resize discarded the active drawing');
      await $('editorUndoInsertion').onclick();
      if (ink() !== pageTwoInk) throw new Error('Completed drawing not undoable');
      await $('editorRedoInsertion').onclick();
      if (ink() <= pageTwoInk) throw new Error('Completed drawing not redoable');
      await $('editorUndoInsertion').onclick();
      const inputKey = new KeyboardEvent('keydown',{key:'z',ctrlKey:true,bubbles:true,cancelable:true});
      $('editorTextInput').dispatchEvent(inputKey);
      if(inputKey.defaultPrevented || ink() !== pageTwoInk) throw new Error('Editor shortcut intercepted text editing');
      $('editorStage').dispatchEvent(new KeyboardEvent('keydown',{key:'z',ctrlKey:true,shiftKey:true,bubbles:true,cancelable:true}));
      await ready(2);
      if(ink() <= pageTwoInk) throw new Error('Editor keyboard redo failed');
      $('editorStage').dispatchEvent(new KeyboardEvent('keydown',{key:'z',ctrlKey:true,bubbles:true,cancelable:true}));
      await ready(2);
      if(ink() !== pageTwoInk) throw new Error('Editor keyboard undo failed');
      // Exporting while comparing must still apply all annotations.
      $('editorShowOriginal').click();
      const { workspaceBridge } = await import('./app.js');
